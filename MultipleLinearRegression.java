...
public class MultipleLinearRegression{
	...
	public static void calculation(String s) {
		...
		if(arguments==2){
			...
			//LR=PT.getWs(x1,x2,x3,y,arguments,LR);
			R=result(LR,P,arguments);
		...
		//MLR=PT.getWs(x1,x2,x3,y,arguments,MLR);
		R=result(MLR,P,arguments);	
		} //end principal else

		System.out.println("Prediccion = "+R+"\n");
		double Sig= PT.Sigmoid(R);
		System.out.println("Y = "+Sig+"\n");
	}

	public static double result(double [] R, double [] P,int arguments) {
		//System.out.print("\nPrediccion: ");
		double prediccion=0;
		int j=0;
		for(int i=0;i<arguments;i++){
